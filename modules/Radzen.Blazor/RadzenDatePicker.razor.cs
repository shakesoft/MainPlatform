...
namespace Radzen.Blazor
{
    ...
    public partial class RadzenDatePicker<TValue> : RadzenComponent, IRadzenFormComponent
    {
        ...
        void UpdateYearsAndMonths(DateTime? min, DateTime? max)
        {
            ...
            var minSupportedYear = Culture.Calendar.MinSupportedDateTime.Year;
            var maxSupportedYear = Culture.Calendar.MaxSupportedDateTime.Year;
            
            YearFrom = Math.Max(minSupportedYear, Math.Min(maxSupportedYear, YearFrom));
            YearTo = Math.Max(minSupportedYear, Math.Min(maxSupportedYear, YearTo));
        ...
        private string FormatYear(int year)
        {
            try
            {
                var minYear = Culture.Calendar.MinSupportedDateTime.Year;
                var maxYear = Culture.Calendar.MaxSupportedDateTime.Year;
            ...
            {
                var safeYear = Math.Max(1900, Math.Min(2100, year));
